...
final class OpenGlModelRenderer {
    ...
    static void render(OpenGlModelInstance target, Entity entityIn, float entityYaw, float entityPitch,
                       Vector3f entityTrans, MatrixStack deliverStack, int packedLight) {
        ...
        deliverStack.rotate(com.shiroha.mmdskin.util.MojangMathBridge.toMojang(workingQuat.identity().rotateY(-yawRad)));
        deliverStack.rotate(com.shiroha.mmdskin.util.MojangMathBridge.toMojang(workingQuat.identity().rotateX(entityPitch * ((float) Math.PI / 180F))));
    ...
    private static boolean initializeToonShaderIfNeeded() {
        // 1.16.5 直移植说明：外置 MMDShader(3d-skin/Shader) 缺失时，classpath 内置
        // toon 程序是唯一可用绘制路径——此时无视 toon 开关强制启用。
        if (!ConfigManager.isToonRenderingEnabled() && OpenGlModelInstance.MMDShaderProgram > 0) {
            return false;
        }
        ...
        if (OpenGlModelInstance.toonShaderCpu == null) {
            synchronized (OpenGlModelInstance.class) {
                if (OpenGlModelInstance.toonShaderCpu == null) {
                    ...
                    if (!shader.init()) {
                        logger.warn("ToonShaderCpu init failed");
                        return false;
                    }
    ...
    private static void renderStandard(OpenGlModelInstance target, Minecraft minecraft,
                                       LightingHelper.LightData light, MatrixStack deliverStack) {
        boolean irisActive = IrisCompat.isIrisShaderActive();
        float colorFactor = irisActive ? 1.0f : light.intensity();
        RenderSystem.color4f(colorFactor, colorFactor, colorFactor, 1.0f);
    ...
    private static boolean bindActiveShader(OpenGlModelInstance target, MatrixStack deliverStack) {
        // 1.16.5 port note: no vanilla core ShaderInstance pipeline; always use the MMD GL program.
        // Skip drawing when the external MMDShader (3d-skin/Shader) is unavailable to avoid GL error spam.
        if (OpenGlModelInstance.MMDShaderProgram <= 0) {
            long now = System.currentTimeMillis();
            if (now - lastMissingShaderLog > 5000L) {
                lastMissingShaderLog = now;
                logger.warn("[mmdskin] MMDShader unavailable (3d-skin/Shader missing); skipping standard-path draw");
            }
            return false;
        }
        target.shaderProgram = OpenGlModelInstance.MMDShaderProgram;
        GlStateManager.useProgram(target.shaderProgram);
        return true;
    }

    private static long lastMissingShaderLog;
    ...
    private static void uploadMatrixUniforms(OpenGlModelInstance target, MatrixStack deliverStack) {
        target.modelViewMatBuff.clear();
        target.projMatBuff.clear();
        deliverStack.getLast().getMatrix().write(target.modelViewMatBuff);
        GL46C.glGetFloatv(org.lwjgl.opengl.GL11.GL_PROJECTION_MATRIX, target.projMatBuff);
    ...
    private static void drawSubMeshes(OpenGlModelInstance target, Minecraft minecraft) {
        int missingTextureId = minecraft.getTextureManager()
                .getTexture(MissingTextureSprite.getLocation())
                .getGlTextureId();
    ...
    private static void renderToon(OpenGlModelInstance target, Minecraft minecraft, float lightIntensity, MatrixStack deliverStack) {
        GL46C.glBindVertexArray(target.vertexArrayObject);
        RenderSystem.enableBlend();
        RenderSystem.enableDepthTest();
        RenderSystem.blendEquation(GL46C.GL_FUNC_ADD);
