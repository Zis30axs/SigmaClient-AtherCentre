...
/** 鏂囦欢鑱岃矗锛氱粯鍒惰皟璇?HUD 骞跺睍绀烘ā鍨嬩笌绾圭悊杩愯鏃惰祫婧愬崰鐢ㄣ€?*/
public class PerformanceHud {
    ...
    public static void render(MatrixStack graphics) {
        ...
        if (minecraft.gameSettings.hideGUI || minecraft.gameSettings.showDebugInfo) {
            return;
        }
        ...
        if (now - lastRefreshTime > REFRESH_INTERVAL_MS) {
            rebuildLines(minecraft.fontRenderer);
            lastRefreshTime = now;
        }
        ...
        net.minecraft.client.gui.AbstractGui.fill(graphics, PADDING, PADDING, PADDING + totalWidth, PADDING + totalHeight, BG_COLOR);

        int y = PADDING + INNER_PAD;
        for (HudLine line : cachedLines) {
            font.drawStringWithShadow(graphics, line.text(), PADDING + INNER_PAD, y, line.color());
            y += LINE_HEIGHT;
        }
    }

    private static void rebuildLines(FontRenderer font) {
        cachedLines.clear();
        ...
        cachedMaxWidth = 0;
        for (HudLine line : cachedLines) {
            cachedMaxWidth = Math.max(cachedMaxWidth, font.getStringWidth(line.text()));
        }
    }
