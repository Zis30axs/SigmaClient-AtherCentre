...
public class MorphWheelScreen extends AbstractWheelScreen {
    ...
    private final KeyBinding triggerKey;
    ...
    public MorphWheelScreen(KeyBinding keyMapping) {
        this(keyMapping, new DefaultMorphWheelService());
    }

    MorphWheelScreen(KeyBinding keyMapping, MorphWheelService morphWheelService) {
        super(new TranslationTextComponent("gui.mmdskin.morph_wheel"), STYLE);
    ...
    protected void init() {
        ...
        this.addButton(createWheelIconButton(new StringTextComponent("⚙"), btn -> {
            this.minecraft.displayGuiScreen(new MorphWheelConfigScreen(this));
        }));
    }
    ...
    public void render(MatrixStack guiGraphics, int mouseX, int mouseY, float partialTick) {
        if (!morphSlots.isEmpty()) {
            ...
            renderEmptyState(guiGraphics, new TranslationTextComponent("gui.mmdskin.morph_wheel.no_morphs"));
        }

        String centerText = new TranslationTextComponent("gui.mmdskin.morph_wheel.select").getString();
    ...
    public boolean keyReleased(int keyCode, int scanCode, int modifiers) {
        if (triggerKey != null) {
            net.minecraft.client.util.InputMappings.Input boundKey = KeyMappingUtil.getBoundKey(triggerKey);
            if (boundKey != null
                    && boundKey.getType() == net.minecraft.client.util.InputMappings.Type.KEYSYM
                    && boundKey.getKeyCode() == keyCode) {
                if (selectedSlot >= 0 && selectedSlot < morphSlots.size()) {
                    executeMorph(morphSlots.get(selectedSlot));
                }
                this.closeScreen();
