...
/** 轮盘界面基类，负责原生 MatrixStack 轮盘渲染与交互动画。 */
public abstract class AbstractWheelScreen extends Screen {
    ...
    protected AbstractWheelScreen(ITextComponent title, WheelStyle style) {
        super(title);
        this.style = style;
    }
    ...
    protected void renderWheelBase(MatrixStack guiGraphics, int mouseX, int mouseY, float partialTick, List<WheelEntry> entries) {
    ...
    protected void renderCenterBubble(MatrixStack guiGraphics, String text, int textColor) {
        ...
        int textWidth = this.font.getStringWidth(display);
        this.font.drawString(guiGraphics, display, centerX - textWidth / 2 + 1, centerY - 4, style.textShadow());
        this.font.drawString(guiGraphics, display, centerX - textWidth / 2, centerY - 5, textColor);
    }

    protected void renderEmptyState(MatrixStack guiGraphics, ITextComponent hint) {
        int width = Math.max(180, this.font.getStringPropertyWidth(hint) + 48);
        int height = 36;
        ...
        drawCenteredString(guiGraphics, this.font, hint, centerX, y + 13, TEXT_PRIMARY);
    }

    protected Button createWheelIconButton(ITextComponent label, Button.IPressable onPress) {
        return new Button(this.width - 36, this.height - 34, 28, 24, label, onPress);
    }

    private void renderBackdrop(MatrixStack guiGraphics) {
        if (this.minecraft == null || this.minecraft.world == null) {
            this.renderBackground(guiGraphics);
        }
        fill(guiGraphics, 0, 0, this.width, this.height, BACKDROP_DIM);
    }

    private void renderSegmentWheel(MatrixStack guiGraphics, List<WheelEntry> entries) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            int maxTextWidth = Math.round(MathHelper.clamp(2.0f * textRadius * (float) Math.sin(halfSweepRad) * 0.90f, 86.0f, 210.0f));
    ...
    private void renderCenterDecor(MatrixStack guiGraphics) {
    ...
    private void renderSingleEntry(MatrixStack guiGraphics, String text, int centerTextX, int centerTextY, float pop, int maxWidth) {
        String display = fitText(text, Math.max(62, maxWidth));
        int width = this.font.getStringWidth(display);
        int color = blendColors(TEXT_MUTED, TEXT_PRIMARY, 0.36f + pop * 0.64f);
        this.font.drawString(guiGraphics, display, centerTextX - width / 2 + 1, centerTextY - 4, style.textShadow());
        this.font.drawString(guiGraphics, display, centerTextX - width / 2, centerTextY - 5, color);
    }

    private void renderDualEntry(MatrixStack guiGraphics, String icon, String label, int centerTextX, int centerTextY, float pop, int maxWidth) {
        String displayIcon = icon == null ? "" : icon;
        int iconWidth = this.font.getStringWidth(displayIcon);
        int iconColor = blendColors(TEXT_MUTED, TEXT_PRIMARY, 0.42f + pop * 0.58f);
        guiGraphics.push();
        guiGraphics.translate(centerTextX, centerTextY - 10.0f, 0.0f);
        guiGraphics.scale(1.0f + pop * 0.08f, 1.0f + pop * 0.08f, 1.0f);
        this.font.drawString(guiGraphics, displayIcon, -iconWidth / 2 + 1, 0, style.textShadow());
        this.font.drawString(guiGraphics, displayIcon, -iconWidth / 2, -1, iconColor);
        guiGraphics.pop();

        String displayLabel = fitText(label, Math.max(56, Math.round(maxWidth * 0.76f)));
        int labelWidth = this.font.getStringWidth(displayLabel);
        int labelColor = blendColors(TEXT_MUTED, TEXT_SECONDARY, 0.52f + pop * 0.38f);
        guiGraphics.push();
        guiGraphics.translate(centerTextX, centerTextY + 4.0f, 0.0f);
        guiGraphics.scale(0.90f + pop * 0.02f, 0.90f + pop * 0.02f, 1.0f);
        this.font.drawString(guiGraphics, displayLabel, -labelWidth / 2 + 1, 0, style.textShadow());
        this.font.drawString(guiGraphics, displayLabel, -labelWidth / 2, -1, labelColor);
        guiGraphics.pop();
    }
    ...
    protected String fitText(String text, int maxWidth) {
        if (text == null || text.isEmpty() || this.font.getStringWidth(text) <= maxWidth) {
            return text == null ? "" : text;
        }
        String trimmed = text;
        while (trimmed.length() > 1 && this.font.getStringWidth(trimmed + "..") > maxWidth) {
    ...
    protected int blendColors(int from, int to, float progress) {
        ...
        int a = MathHelper.floor(MathHelper.lerp(clamped, alpha(from), alpha(to)));
        int r = MathHelper.floor(MathHelper.lerp(clamped, red(from), red(to)));
        int g = MathHelper.floor(MathHelper.lerp(clamped, green(from), green(to)));
        int b = MathHelper.floor(MathHelper.lerp(clamped, blue(from), blue(to)));
    ...
    private void fillRoundedRect(MatrixStack guiGraphics, int x, int y, int width, int height, int radius, int color) {
        ...
        for (int row = 0; row < height; row++) {
            int inset = row < clampedRadius
                    ...
                    : 0;
            fill(guiGraphics,x + inset, y + row, x + width - inset, y + row + 1, color);
        }
    }

    private void fillCircle(MatrixStack guiGraphics, int cx, int cy, int radius, int color) {
        if (radius <= 0) {
            return;
        }
        for (int dy = -radius; dy <= radius; dy++) {
            int span = (int) Math.sqrt(radius * radius - dy * dy);
            fill(guiGraphics,cx - span, cy + dy, cx + span + 1, cy + dy + 1, color);
        }
    }

    private void drawCircleOutline(MatrixStack guiGraphics, int cx, int cy, int radius, int color) {
        if (radius <= 1) {
            return;
        }
        for (int dy = -radius; dy <= radius; dy++) {
            ...
            fill(guiGraphics,cx - outer, cy + dy, cx - inner, cy + dy + 1, color);
            fill(guiGraphics,cx + inner + 1, cy + dy, cx + outer + 1, cy + dy + 1, color);
        }
    }

    private void drawCircleArcOutline(MatrixStack guiGraphics, float cx, float cy, float radius, int color, float width) {
        drawRing(guiGraphics, cx, cy, Math.max(0.0f, radius - width), radius, 0.0f, 360.0f, color);
    }

    private void drawSeparator(MatrixStack guiGraphics, float cx, float cy, float inner, float outer, float angleDeg, int color) {
        float radians = (float) Math.toRadians(angleDeg);
        float dx = MathHelper.cos(radians);
        float dy = MathHelper.sin(radians);
        float nx = -dy * 0.9f;
        float ny = dx * 0.9f;
    ...
    private void drawAnnularSegment(MatrixStack guiGraphics, float cx, float cy,
    ...
    private void drawAnnularSegmentOutline(MatrixStack guiGraphics, float cx, float cy,
    ...
    private void drawRing(MatrixStack guiGraphics, float cx, float cy,
                          float inner, float outer, float startDeg, float sweepDeg, int color) {
        ...
        Matrix4f matrix = guiGraphics.getLast().getMatrix();
    ...
    private void drawQuad(MatrixStack guiGraphics,
                          float ax, float ay,
                          float bx, float by,
                          float cx, float cy,
                          float dx, float dy,
