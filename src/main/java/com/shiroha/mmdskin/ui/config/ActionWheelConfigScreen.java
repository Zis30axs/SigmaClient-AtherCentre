...
public class ActionWheelConfigScreen extends Screen {
    ...
    public ActionWheelConfigScreen(Screen parent) {
        super(new TranslationTextComponent("gui.mmdskin.action_config"));
        this.parent = parent;
        reloadState();
    }
    ...
    private void updateLayout() {
        ...
        int panelWidth = MathHelper.clamp(Math.round(this.width * WINDOW_WIDTH_RATIO), minPanelWidth, maxPanelWidth);
        int panelHeight = MathHelper.clamp(Math.round(this.height * WINDOW_HEIGHT_RATIO), minPanelHeight, maxPanelHeight);
    ...
    private void renderScreen(MatrixStack guiGraphics) {
        ...
        drawColumn(guiGraphics, layout.availableColumn, layout.availableList,
                new TranslationTextComponent("gui.mmdskin.action_config.available").getString(),
        ...
        drawColumn(guiGraphics, layout.selectedColumn, layout.selectedList,
                new TranslationTextComponent("gui.mmdskin.action_config.selected").getString(),
                state.selectedItems(), hoveredSelectedIndex, selectedAnimatedScroll);
        drawFooter(guiGraphics);
    }

    private void drawHeader(MatrixStack guiGraphics) {
        this.font.drawString(guiGraphics, this.title.getString(), layout.header.x, layout.header.y + 2, TranslucentTrayChrome.TITLE_TEXT);
        ITextComponent stats = new TranslationTextComponent("gui.mmdskin.config.stats", state.availableCount(), state.selectedCount());
        this.font.drawString(guiGraphics, stats.getString(), layout.header.x, layout.header.y + 16, TranslucentTrayChrome.SUBTITLE_TEXT);
        this.font.drawString(guiGraphics,
                new TranslationTextComponent("gui.mmdskin.action_wheel.click_select").getString(),
                layout.header.x, layout.header.y + 28, TranslucentTrayChrome.MUTED_TEXT);
    }

    private void drawColumn(MatrixStack guiGraphics,
                            UiRect columnRect,
                            UiRect listRect,
                            String title,
        ...
        this.font.drawString(guiGraphics, title, columnRect.x + 6, columnRect.y + 6, TranslucentTrayChrome.TITLE_TEXT);
        ...
        if (items.isEmpty()) {
            drawCenteredString(guiGraphics, this.font, "-", listRect.centerX(), listRect.centerY() - 4, TranslucentTrayChrome.DIM_TEXT);
            return;
        }

        com.shiroha.mmdskin.ui.ScissorCompat.enable(listRect.x, listRect.y, listRect.x + listRect.w, listRect.y + listRect.h);
        ...
        com.shiroha.mmdskin.ui.ScissorCompat.disable();
        drawScrollBar(guiGraphics, listRect, items.size(), scrollOffset);
    }

    private void drawEntryCard(MatrixStack guiGraphics, UiRect listRect, int y,
                               ActionWheelConfig.ActionEntry entry, boolean hovered) {
        int x = listRect.x + 4;
        int w = listRect.w - 12;
        fill(guiGraphics, x, y, x + w, y + CARD_HEIGHT, TranslucentTrayChrome.cardBackground(false, hovered));
        fill(guiGraphics, x, y, x + 2, y + CARD_HEIGHT, hovered ? TranslucentTrayChrome.ACCENT_STRIP_ACTIVE : TranslucentTrayChrome.ACCENT_STRIP);
        this.font.drawString(guiGraphics, buildEntryTitle(entry), x + 8, y + 4, TranslucentTrayChrome.BODY_TEXT);
        this.font.drawString(guiGraphics, buildEntryMeta(entry), x + 8, y + 15, TranslucentTrayChrome.DETAIL_TEXT);
    }

    private void drawFooter(MatrixStack guiGraphics) {
        drawButton(guiGraphics, layout.refreshButton, new TranslationTextComponent("gui.mmdskin.refresh").getString(), hoveredButton == ButtonTarget.REFRESH);
        drawButton(guiGraphics, layout.selectAllButton, new TranslationTextComponent("gui.mmdskin.select_all").getString(), hoveredButton == ButtonTarget.SELECT_ALL);
        drawButton(guiGraphics, layout.clearAllButton, new TranslationTextComponent("gui.mmdskin.clear_all").getString(), hoveredButton == ButtonTarget.CLEAR_ALL);
        drawButton(guiGraphics, layout.saveButton, new TranslationTextComponent("gui.done").getString(), hoveredButton == ButtonTarget.SAVE);
        drawButton(guiGraphics, layout.cancelButton, new TranslationTextComponent("gui.cancel").getString(), hoveredButton == ButtonTarget.CANCEL);
    }

    private void drawButton(MatrixStack guiGraphics, UiRect rect, String text, boolean hovered) {
        TranslucentTrayChrome.drawButton(guiGraphics, this.font, rect.x, rect.y, rect.w, rect.h, text, hovered, true);
    }

    private void drawPanel(MatrixStack guiGraphics, UiRect rect) {
        TranslucentTrayChrome.drawPanel(guiGraphics, rect.x, rect.y, rect.w, rect.h);
    }

    private void drawScrollBar(MatrixStack guiGraphics, UiRect listRect, int itemCount, float scrollOffset) {
    ...
    private float clampScroll(float scroll, float maxScroll) {
        return MathHelper.clamp(scroll, 0.0f, maxScroll);
    }
