...
public final class TranslucentTrayChrome {
    ...
    public static void drawOverlay(MatrixStack graphics, int screenWidth, int screenHeight) {
        net.minecraft.client.gui.AbstractGui.fill(graphics, 0, 0, screenWidth, screenHeight, OVERLAY);
    }

    public static void drawPanel(MatrixStack graphics, int x, int y, int width, int height) {
        net.minecraft.client.gui.AbstractGui.fill(graphics, x, y, x + width, y + height, PANEL_OUTER);
        if (width > 2 && height > 2) {
            net.minecraft.client.gui.AbstractGui.fill(graphics, x + 1, y + 1, x + width - 1, y + height - 1, PANEL_INNER);
        }
    }

    public static void fillListArea(MatrixStack graphics, int x, int y, int width, int height) {
        net.minecraft.client.gui.AbstractGui.fill(graphics, x, y, x + width, y + height, LIST_BACKGROUND);
    }

    public static void drawSeparator(MatrixStack graphics, int x, int y, int width) {
        net.minecraft.client.gui.AbstractGui.fill(graphics, x, y, x + width, y + 1, SEPARATOR);
    }

    public static void drawButton(MatrixStack graphics, FontRenderer font,
        ...
        net.minecraft.client.gui.AbstractGui.fill(graphics, x, y, x + width, y + height, background);
        net.minecraft.client.gui.AbstractGui.drawCenteredString(graphics, font, text, x + width / 2, y + Math.max(0, (height - font.FONT_HEIGHT) / 2), textColor);
    }

    public static void drawButton(MatrixStack graphics, FontRenderer font,
                                  int x, int y, int width, int height,
                                  ITextComponent text, boolean hovered, boolean enabled) {
        drawButton(graphics, font, x, y, width, height, text.getString(), hovered, enabled);
    }

    public static void drawScrollbar(MatrixStack graphics, int x, int top, int bottom, float offset, float maxScroll) {
        if (maxScroll <= 0.0f) {
            return;
        }
        int barHeight = bottom - top;
        net.minecraft.client.gui.AbstractGui.fill(graphics, x, top, x + 2, bottom, SCROLL_TRACK);
        ...
        net.minecraft.client.gui.AbstractGui.fill(graphics, x, thumbY, x + 2, thumbY + thumbHeight, SCROLL_THUMB);
    }
