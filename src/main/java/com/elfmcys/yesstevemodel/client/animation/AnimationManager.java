...
public class AnimationManager implements IAnimationPredicate<CustomPlayerEntity> {
    ...
    private static final ReferenceArrayList<AnimationState<PlayerEntity, CustomPlayerEntity>>[] data = new ReferenceArrayList[Priority.LOWEST + 1];
    ...
    public static void register(AnimationState<PlayerEntity, CustomPlayerEntity> state) {
    ...
    public PlayState predicate(AnimationEvent<CustomPlayerEntity> event, ExpressionEvaluator<?> evaluator) {
        PlayerEntity player = event.getAnimatable().getEntity();
        if (player == null) {
        ...
        for (int i = Priority.HIGHEST; i <= Priority.LOWEST; i++) {
            for (AnimationState<PlayerEntity, CustomPlayerEntity> animationState : data[i]) {
