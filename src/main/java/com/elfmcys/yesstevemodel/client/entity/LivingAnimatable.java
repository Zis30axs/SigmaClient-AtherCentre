...
public abstract class LivingAnimatable<T extends LivingEntity> extends GeoEntity<T> {
    ...
    private void updateCurrentTexture() {
        if (isModelReady()) {
            OrderedStringMap<String, ? extends Texture> map = getModelAssembly().getAnimationBundle().getTextures();
            Texture Texture = map.get(this.currentTextureName);
            if (Texture != null) {
                ((TexturedModelWrapper) getRenderShape()).setTexture(Texture);
                this.textureIndex = map.getValuesList().indexOf(Texture);
            } else {
    ...
    public class TexturedModelWrapper extends ModelWrapper {
        ...
        public TexturedModelWrapper(ModelAssembly modelAssembly, boolean isActive, boolean collectAllTextures, boolean registerImmediately, int textureResolution) {
            ...
            Texture Texture = modelAssembly.getAnimationBundle().getTextures().get(LivingAnimatable.this.currentTextureName);
            this.currentTexture = UploadManager.getOrCreateLocatableWithSize(Texture != null ? Texture : modelAssembly.getAnimationBundle().getDefaultTexture(), registerImmediately, textureResolution);
            this.textureResolution = textureResolution;
            if (collectAllTextures) {
                this.allTextures = new ArrayList();
                for (Texture texture : modelAssembly.getAnimationBundle().getTextures().values()) {
        ...
        private void setTexture(Texture Texture) {
            this.currentTexture = UploadManager.getOrCreateLocatableWithSize(Texture, true, this.textureResolution);
        }
