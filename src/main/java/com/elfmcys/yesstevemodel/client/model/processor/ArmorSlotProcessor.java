...
public class ArmorSlotProcessor<T extends GeoEntity<?>> implements ModelProcessor<T, PlayerModelBundle> {
    ...
    private final TriFunction<String, T, EquipmentSlotType, IAnimationController<T>> controllerFactory;

    public ArmorSlotProcessor(String prefix, String category, AnimationDataProvider<PlayerModelBundle> animationDataProvider, TriFunction<String, T, EquipmentSlotType, IAnimationController<T>> controllerFactory) {
        this.prefix = prefix;
    ...
    public ControllerFactory<T> process(PlayerModelBundle modelBundle, ModelResourceBundle resourceBundle) {
        ArrayList<Pair<String, EquipmentSlotType>> matchingSlots = new ArrayList<>();
        ...
        for (EquipmentSlotType slot : EquipmentSlotType.values()) {
            ...
            if (animationEntries.containsKey(slotKey)) {
                ...
                matchingSlots.add(Pair.of(slotKey, slot));
            } else if (slot.getSlotType() == EquipmentSlotType.Group.ARMOR && (armorCondition.hasFilter(slot) || animations.containsKey(slot.getName() + ":default"))) {
                matchingSlots.add(Pair.of(slotKey, slot));
            }
        }
        return (entity, consumer) -> {
            if (!(entity instanceof IPreviewAnimatable)) {
                for (Pair<String, EquipmentSlotType> pair : matchingSlots) {
