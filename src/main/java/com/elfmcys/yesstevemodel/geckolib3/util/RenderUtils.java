...
public final class RenderUtils {
    public static void translateMatrixToBone(MatrixStack poseStack, IBone bone) {
        poseStack.translate((-bone.getPositionX()) / 16f, bone.getPositionY() / 16f, bone.getPositionZ() / 16f);
    }

    public static void rotateMatrixAroundBone(MatrixStack poseStack, IBone bone) {
        if (bone.getRotationZ() != 0.0F || bone.getRotationY() != 0.0F || bone.getRotationX() != 0.0F) {
            Quaternionf qf = new Quaternionf().rotateZYX(bone.getRotationZ(), bone.getRotationY(), bone.getRotationX());
            poseStack.rotate(new net.minecraft.util.math.vector.Quaternion(qf.x, qf.y, qf.z, qf.w));
        }
    }

    public static boolean scaleMatrixForBone(MatrixStack poseStack, IBone bone) {
    ...
    public static void translateToPivotPoint(MatrixStack poseStack, IBone bone) {
        poseStack.translate(bone.getPivotX() / 16f, bone.getPivotY() / 16f, bone.getPivotZ() / 16f);
    }

    public static void translateAwayFromPivotPoint(MatrixStack poseStack, IBone bone) {
        poseStack.translate(-bone.getPivotX() / 16f, -bone.getPivotY() / 16f, -bone.getPivotZ() / 16f);
    }

    public static void translateAndRotateMatrixForBone(MatrixStack poseStack, IBone bone) {
        translateToPivotPoint(poseStack, bone);
        rotateMatrixAroundBone(poseStack, bone);
    }

    public static boolean prepMatrixForBone(MatrixStack poseStack, IBone bone) {
    ...
    public static boolean prepMatrixForLocator(MatrixStack poseStack, List<? extends IBone> locatorHierarchy) {
