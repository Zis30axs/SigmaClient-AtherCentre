    ...
    public IValue parseExpression(String molangExpression, boolean isScript) {
        try {
            ...
            if (AnimationDebugOverlay.isDebugActive()) {
                ...
                ChatLogger.INSTANCE.logComponent(new TranslationTextComponent("error.yes_steve_model.parse_molang_exp").appendString(e.getMessage()).appendString("\n----------------------\n").appendString(molangExpression.replace("\r\n", "\n").replace("\r", "\n")).appendString("\n----------------------"));
            } else {
