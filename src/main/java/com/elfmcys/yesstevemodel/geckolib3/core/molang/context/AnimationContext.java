...
public class AnimationContext<TEntity> implements IContext<TEntity> {
    ...
    private AnimationContext(TEntity entity, AnimationContext<?> context) {
        ...
        // Foreign (`p.`-style cross-entity) variable storage: the port has its own capability
        // registry, so this is the upstream lookup with Forge's LazyOptional swapped for Optional.
        if (entity instanceof PlayerEntity) {
            ((PlayerEntity) entity).getCapability(PlayerCapabilityProvider.PLAYER_CAP)
                    .ifPresent(cap -> this.foreignStorage = cap.getPropertyGetter());
        } else if (entity instanceof ProjectileEntity) {
            ((ProjectileEntity) entity).getCapability(ProjectileCapabilityProvider.PROJECTILE_CAP)
                    .ifPresent(cap -> this.foreignStorage = cap.getPropertyGetter());
        } else if (entity instanceof Entity) {
            ((Entity) entity).getCapability(VehicleCapabilityProvider.VEHICLE_CAP)
                    .ifPresent(cap -> this.foreignStorage = cap.getPropertyGetter());
        }
    }
    ...
    @Override
    public void logWarningComponent(ITextComponent ITextComponent) {
        if (isDebugMode()) {
            this.logger.logComponent(ITextComponent);
        }
    }
