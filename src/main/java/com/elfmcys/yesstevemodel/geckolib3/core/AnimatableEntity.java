...
public abstract class AnimatableEntity<TEntity extends Entity> {
    ...
    public int getRefreshRate() {
        ...
        if (tentity != null && tentity != this.entity) {
            Vector3d vec3Position = tentity.getPositionVec();
            if (vec3Position.x != 0.0d || vec3Position.y != 0.0d || vec3Position.z != 0.0d) {
                if (!this.isFirstFrameAfterReset) {
                    return 10;
                }
                float fDistanceTo = tentity.getDistance(this.entity);
    ...
    public AnimationEvent<?> processAnimationImpl(float partialTick, boolean z) {
        ...
        int tickCount = this instanceof IPreviewAnimatable ? ClientTickEvent.getTickCount() : entity.ticksExisted;
        float frameTime = partialTick != 1.0f ? partialTick : Minecraft.getInstance().getRenderPartialTicks();
        boolean shouldSit = entity.isPassenger() && entity.getRidingEntity() != null && entity.getRidingEntity() instanceof net.minecraft.entity.LivingEntity;
        ...
        if (livingEntity != null) {
            modelData.isChild = livingEntity.isChild();
            lerpBodyRot = MathHelper.interpolateAngle(partialTick, livingEntity.prevRenderYawOffset, livingEntity.renderYawOffset);
            lerpHeadRot = MathHelper.interpolateAngle(partialTick, livingEntity.prevRotationYawHead, livingEntity.rotationYawHead);
            netHeadYaw = lerpHeadRot - lerpBodyRot;
        }

        if (shouldSit && (entity.getRidingEntity() instanceof LivingEntity vehicle)) {
            lerpBodyRot = MathHelper.interpolateAngle(partialTick, vehicle.prevRenderYawOffset, vehicle.renderYawOffset);
            netHeadYaw = lerpHeadRot - lerpBodyRot;
            float clampedHeadYaw = MathHelper.clamp(MathHelper.wrapDegrees(lerpHeadRot - lerpBodyRot), -85.0f, 85.0f);
        ...
        modelData.rawHeadPitch = MathHelper.lerp(partialTick, entity.prevRotationPitch, entity.rotationPitch);
        modelData.headPitch = -modelData.rawHeadPitch;
        modelData.rawNetHeadYaw = netHeadYaw;
        modelData.netHeadYaw = -MathHelper.clamp(MathHelper.wrapDegrees(netHeadYaw), -85.0f, 85.0f);
        ...
        if (!this.animationProcessor.isDisabled()) {
            ...
            if (z2) {
                if (z3) {
                    ...
                    this.positionTracker.updateState(event.getTickCount(), this.seekTime, event.getPartialTick());
                }
    ...
    public boolean isDebugMode() {
        return Minecraft.getInstance().world == this.entity.world && !this.entity.removed;
    }
