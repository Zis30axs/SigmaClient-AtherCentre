...
/**
 * Upstream {@code audio/AudioStreamWrapper} (verbatim modulo MCP stream method names): looping
 * wrapper that re-opens the factory stream each time the current one runs dry.
 */
public class AudioStreamWrapper implements IAudioStreamSupport {
    ...
    public AudioStreamWrapper(IAudioStreamFactory streamFactory2) throws UnsupportedAudioFileException, IOException {
        ...
        try {
            ...
            this.audioFormat = this.currentStream.getAudioFormat();
    ...
    public ByteBuffer readOggSoundWithCapacity(int i) throws IOException {
        if (this.currentStream != null) {
            ByteBuffer byteBuffer = this.currentStream.readOggSoundWithCapacity(i);
            if (byteBuffer.remaining() == 0) {
                this.currentStream = null;
                try {
                    reset();
                    this.currentStream = this.streamFactory.openStream();
                    byteBuffer = this.currentStream.readOggSoundWithCapacity(i);
                    if (byteBuffer.remaining() == 0) {
                        reset();
                        return EMPTY_BUFFER;
                    }
