...
/**
 * Upstream {@code audio/OggVorbisAudioStream}. The decoder is vanilla's own
 * {@code net.minecraft.client.audio.OggAudioStream} (stb_vorbis), which is exactly what
 * upstream's {@code com.mojang.blaze3d.audio.OggAudioStream} is; only the MCP names differ
 * ({@code getFormat} -> {@code getAudioFormat}, {@code read(int)} ->
 * {@code readOggSoundWithCapacity(int)}). Stereo is down-mixed to mono, matching upstream.
 */
public class OggVorbisAudioStream implements IAudioStreamSupport {
    ...
    public OggVorbisAudioStream(ByteBuffer byteBuffer, @Nullable AudioCacheBuilder cacheBuilder) throws UnsupportedAudioFileException, IOException {
        ...
        if (this.oggStream.getAudioFormat().getChannels() != 1 && this.oggStream.getAudioFormat().getChannels() != 2) {
            throw new UnsupportedAudioFileException();
        }
        this.audioFormat = new AudioFormat(this.oggStream.getAudioFormat().getSampleRate(), 16, 1, true, false);
    ...
    public ByteBuffer readOggSoundWithCapacity(int i) throws IOException {
        ...
        ByteBuffer byteBufferSlice = this.oggStream.readOggSoundWithCapacity(this.oggStream.getAudioFormat().getChannels() * i);
        ...
        if (this.oggStream.getAudioFormat().getChannels() == 2) {
