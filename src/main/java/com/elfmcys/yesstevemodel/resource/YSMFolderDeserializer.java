...
// PORT-REVIEW (1.20.1 -> 1.16.5): faithful port of OpenYSM 2.6.5 YSMFolderDeserializer.
// Adaptations: commons-codec DigestUtils -> local MessageDigest helpers (md5Hex/sha256Hex);
// rip.ysm.imagestream JPEG/WebP/AVIF decoders -> ImageIO (PNG/BMP/JPEG native; WebP/AVIF
// dimensions fall back to 0x0 since those decoders are not ported yet).
...
public class YSMFolderDeserializer implements AutoCloseable {
    ...
    private static ImageMeta parseImageMeta(byte[] data, String path) {
        ...
        try {
            ...
            switch (format) {
                ...
                case 3 -> img = ImageIO.read(new ByteArrayInputStream(data));                 // JPEG
                case 4 -> img = ImageIO.read(new ByteArrayInputStream(data));                 // WEBP
                case 5 -> img = ImageIO.read(new ByteArrayInputStream(data));                 // AVIF
            }
            if (img != null) {
                return new ImageMeta(img.getWidth(), img.getHeight(), format);
            }
            return new ImageMeta(0, 0, format); // PORT-REVIEW: WEBP/AVIF dimensions need rip.ysm.imagestream decoders (not ported)
        } catch (Exception e) {
    ...
    private static String hexDigest(String algorithm, byte[] data) {
        try {
