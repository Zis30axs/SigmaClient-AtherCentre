...
public class Chunk implements IChunk {
    ...
    private final ChunkSection[] sections = new ChunkSection[WorldHeightHelper.getSectionCount()];
    ...
    private final ShortList[] packedBlockPositions = new ShortList[WorldHeightHelper.getSectionCount()];
    ...
    public Chunk(World worldIn, ChunkPos chunkPosIn, BiomeContainer biomeContainerIn) {
        this(worldIn, chunkPosIn, biomeContainerIn, UpgradeData.EMPTY, EmptyTickList.get(), EmptyTickList.get(), 0L,
                (ChunkSection[]) null, (Consumer<Chunk>) null);
    }

    public Chunk(World worldIn, ChunkPos chunkPosIn, BiomeContainer biomeContainerIn, UpgradeData upgradeDataIn,
            ITickList<Block> tickBlocksIn, ITickList<Fluid> tickFluidsIn, long inhabitedTimeIn,
            @Nullable ChunkSection[] sectionsIn, @Nullable Consumer<Chunk> postLoadConsumerIn) {
        this.entityLists = new ClassInheritanceMultiMap[WorldHeightHelper.getSectionCount()];
        this.world = worldIn;
        this.pos = chunkPosIn;
        ...
        if (sectionsIn != null) {
            ...
            } else if (sectionsIn.length == 16) {
                System.arraycopy(sectionsIn, 0, this.sections, 4, 16);
            } else {
                LOGGER.warn("Could not set level chunk sections, array length is {} instead of {}", sectionsIn.length,
    ...
    public Chunk(World worldIn, ChunkPrimer primer) {
        this(worldIn, primer.getPos(), primer.getBiomes(), primer.getUpgradeData(), primer.getBlocksToBeTicked(),
                primer.getFluidsToBeTicked(), primer.getInhabitedTime(), primer.getSections(), (Consumer<Chunk>) null);
    ...
    public BlockState getBlockState(BlockPos pos) {
        ...
        if (this.world.isDebug()) {
            ...
            try {
                if (!WorldHeightHelper.isYOutOfBounds(j)) {
                    ChunkSection chunksection = this.sections[WorldHeightHelper.blockYToSectionIndex(j)];
    ...
    public FluidState getFluidState(int bx, int by, int bz) {
        try {
            if (!WorldHeightHelper.isYOutOfBounds(by)) {
                ChunkSection chunksection = this.sections[WorldHeightHelper.blockYToSectionIndex(by)];
    ...
    public BlockState setBlockState(BlockPos pos, BlockState state, boolean isMoving) {
        ...
        if (WorldHeightHelper.isYOutOfBounds(j))
            return null;
        int k = pos.getZ() & 15;
        int sectionIndex = WorldHeightHelper.blockYToSectionIndex(j);
        ...
        if (chunksection == EMPTY_SECTION) {
            ...
            chunksection = new ChunkSection(WorldHeightHelper.sectionToBlockY(sectionIndex));
    ...
    public void addEntity(Entity entityIn) {
        ...
        if (k < WorldHeightHelper.getMinSection()) {
            k = WorldHeightHelper.getMinSection();
        }

        int sectionIndex = WorldHeightHelper.sectionToIndex(k);

        if (sectionIndex >= this.entityLists.length) {
            sectionIndex = this.entityLists.length - 1;
        }
    ...
    public void removeEntityAtIndex(Entity entityIn, int index) {
        int sectionIndex = WorldHeightHelper.sectionToIndex(index);
    ...
    public int getTopBlockY(Heightmap.Type heightmapType, int x, int z) {
        return Math.max(this.heightMap.get(heightmapType).getHeight(x & 15, z & 15) - 1, WorldHeightHelper.getMinY()); // Allow
                                                                                                                       // negative
                                                                                                                       // Y
                                                                                                                       // (I
        // hope i am doing this
        // right)
    }
    ...
    public void getEntitiesWithinAABBForEntity(@Nullable Entity entityIn, AxisAlignedBB aabb, List<Entity> listToFill,
        ...
        i = WorldHeightHelper.sectionToIndex(i); // offset
        j = WorldHeightHelper.sectionToIndex(j); // offset
            ...
            for (int i1 = 0; i1 < l; ++i1) {
                ...
                if (entity.getBoundingBox().intersects(aabb) && entity != entityIn) {
                    ...
                    if (entity instanceof EnderDragonEntity) {
                        for (EnderDragonPartEntity enderdragonpartentity : ((EnderDragonEntity) entity)
                                .getDragonParts()) {
                            if (enderdragonpartentity != entityIn
                                    && enderdragonpartentity.getBoundingBox().intersects(aabb)
                                    && (filter == null || filter.test(enderdragonpartentity))) {
    ...
    public <T extends Entity> void getEntitiesWithinAABBForList(@Nullable EntityType<?> entitytypeIn,
            AxisAlignedBB aabb, List<? super T> list, Predicate<? super T> filter) {
        int i = MathHelper.floor((aabb.minY - 2.0D) / 16.0D);
        int j = MathHelper.floor((aabb.maxY + 2.0D) / 16.0D);
        i = WorldHeightHelper.sectionToIndex(i);
        j = WorldHeightHelper.sectionToIndex(j);
        i = MathHelper.clamp(i, 0, this.entityLists.length - 1);
        j = MathHelper.clamp(j, 0, this.entityLists.length - 1);

        for (int k = i; k <= j; ++k) {
            for (Entity entity : this.entityLists[k].getByClass(Entity.class)) {
                if ((entitytypeIn == null || entity.getType() == entitytypeIn)
                        && entity.getBoundingBox().intersects(aabb) && filter.test((T) entity)) {
                    list.add((T) entity);
                }
            }
        }
    ...
    public <T extends Entity> void getEntitiesOfTypeWithinAABB(Class<? extends T> entityClass, AxisAlignedBB aabb,
            List<T> listToFill, @Nullable Predicate<? super T> filter) {
        int i = MathHelper.floor((aabb.minY - 2.0D) / 16.0D);
        int j = MathHelper.floor((aabb.maxY + 2.0D) / 16.0D);
        i = WorldHeightHelper.sectionToIndex(i);
        j = WorldHeightHelper.sectionToIndex(j);
    ...
    public void read(@Nullable BiomeContainer biomeContainerIn, PacketBuffer packetBufferIn, CompoundNBT nbtIn,
        ...
        if (WorldHeightHelper.isExtendedHeight()
            ...
            LOGGER.info("[ChunkDebug] Chunk({},{}) availableSections=0x{} (binary={}), sectionCount={}, offset={}",
                    ...
                    Integer.toBinaryString(availableSections),
    ...
    /**
     * Sets up or deserializes a {@link TileEntity} at the desired location from the
     * given compound. If the compound's
     * TileEntity id is {@code "DUMMY"}, the TileEntity may be created by the
     * {@link ITileProvider} instance if the
     * {@link Block} at the given position is in fact a provider. Otherwise, the
     * TileEntity is deserialized at the given
     * position.
     */
    private TileEntity setDeferredTileEntity(BlockPos pos, CompoundNBT compound) {
        ...
        if ("DUMMY".equals(compound.getString("id"))) {
            ...
            if (block instanceof ITileEntityProvider) {
                ...
                tileentity = null;
                LOGGER.warn("Tried to load a DUMMY block entity @ {} but found not block entity block {} at location",
                        pos, blockstate);
            }
        } else {
    ...
    public void rescheduleTicks() {
        if (this.blocksToBeTicked instanceof ChunkPrimerTickList) {
            ((ChunkPrimerTickList<Block>) this.blocksToBeTicked).postProcess(this.world.getPendingBlockTicks(),
                    (pos) -> {
        ...
        if (this.fluidsToBeTicked instanceof ChunkPrimerTickList) {
            ((ChunkPrimerTickList<Fluid>) this.fluidsToBeTicked).postProcess(this.world.getPendingFluidTicks(),
                    (pos) -> {
    ...
    /**
     * Remove scheduled ticks belonging to this chunk from the world and keep it
     * locally for incoming serialization
     */
    public void saveScheduledTicks(ServerWorld serverWorldIn) {
        if (this.blocksToBeTicked == EmptyTickList.<Block>get()) {
            this.blocksToBeTicked = new SerializableTickList<>(Registry.BLOCK::getKey,
                    serverWorldIn.getPendingBlockTicks().getPending(this.pos, true, false),
                    serverWorldIn.getGameTime());
            this.setModified(true);
        }

        if (this.fluidsToBeTicked == EmptyTickList.<Fluid>get()) {
            this.fluidsToBeTicked = new SerializableTickList<>(Registry.FLUID::getKey,
                    serverWorldIn.getPendingFluidTicks().getPending(this.pos, true, false),
                    serverWorldIn.getGameTime());
            this.setModified(true);
        }
    }
